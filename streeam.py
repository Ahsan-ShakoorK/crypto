...
def fetch_trading_data(coin):
    default_timezone = pytz.timezone('GMT')
...
def fetch_daily_data_combined(coin, selected_date, timeframe, value=None, mode='highlight'):
    ...
    query = f"""
        SELECT price,
            {', '.join([f"SUM(CASE WHEN HOUR(timestamp) = {interval} AND MINUTE(timestamp) >= 0 AND MINUTE(timestamp) < 60 THEN volume ELSE 0 END) AS volume_{interval}{timeframe}" for interval in interval_list])}
        FROM {coin}usdt
    ...
    # Add the 'price' column to the DataFrame if it's not present in the fetched data
...
def main():
    ...
    st.write("Market data retrieved from MySQL")

    # Get the list of coins
    coins = ["binance_chess", "chess", "blz", "joe", "perl", "ach", "gmt", "xrp", "akro", "zil", "cfx", "adx", "chz", "bel", "alpaca", "elf", "epx", "pros", "t", "dar", "agix", "mob", "id", "trx", "key", "tru", "amb", "magic", "lina", "lever"]
    ...
    if percentage_enabled:
        ...
        df_daily = fetch_daily_data_combined(selected_coin, selected_date, selected_timeframe, value=percentage_value, mode='percentage')
